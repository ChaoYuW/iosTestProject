...
+ (Class)layerClass {
    // Listing 6.2  在UIView 重写layerClass 类方法可以让开发者创建视图实例自定义图层
    //重写layerClass方法并放回 AVCaptureVideoPreviewLayer类对象
    return [AVCaptureVideoPreviewLayer class];
}
- (AVCaptureSession*)session {
    // Listing 6.2 重写session方法 返回捕捉会话
    return [(AVCaptureVideoPreviewLayer*)self.layer session];
}
- (void)setSession:(AVCaptureSession *)session {
    // Listing 6.2   重写session属性的访问方法 在setSession:方法中访问视图layer属性
//    AVCaptureVideoPreviewLayer 实例，并且设置AVCaptureSession 将捕捉数据直接输出到图层中 确保与会话状态同步
